...
namespace PowerOverhauled
{
    public class RealisticWire : KMonoBehaviour, ISim1000ms
    {
        ...
        [SerializeField] public string[] conductorLabels = new[] { "L1", "N" };
        ...
        public void Sim1000ms(float dt)
        {
            ...
            if (overloaded)
                Debug.LogWarning($"[PO][Wire {Cell}] Überlast: {current_A:F1}A > {maxCurrent_A:F1}A");
        }

        public void SetCurrent(float amps) => current_A = Mathf.Max(amps, 0f);
        public float GetCurrent() => current_A;
        public float GetVoltDrop() => voltDrop_V;
        public bool IsOverloaded() => overloaded;

        public string GetInfoString()
        {
            return $"{crossSection_mm2}mm² {numConductors}-adrig [{string.Join("/", conductorLabels)}]: {current_A:F1}A/{maxCurrent_A:F1}A, ΔU={voltDrop_V:F2}V";
        }
    }
}
